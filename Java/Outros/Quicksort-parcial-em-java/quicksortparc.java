...
public class quicksortparc
{
    ...
    private static void quicksort(Personagem[] p,int esq, int dir, int k)
    {
        int i = esq;
        int j = dir;
        ...
        while (i <= j)
        {
            ...
            while (p[i].getcorDoCabelo().compareTo(pivo.getcorDoCabelo())<0 ||
            (p[i].getcorDoCabelo().compareTo(pivo.getcorDoCabelo())==0 && p[i].getnome().compareTo(pivo.getnome())<0))
            {
                numcpr++;
                i++;
            }

            while (p[j].getcorDoCabelo().compareTo(pivo.getcorDoCabelo())>0 ||
            (p[j].getcorDoCabelo().compareTo(pivo.getcorDoCabelo())==0 && p[j].getnome().compareTo(pivo.getnome())>0))
            {
                numcpr++;
                j--;
            }
    ...
    public static void imprimir(Personagem[] p, int n, int k)throws Exception
    {
