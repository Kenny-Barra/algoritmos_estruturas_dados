...
public class countingsort
{
    ...
    {
        FileWriter arquivo = new FileWriter("matricula_countingsort.txt");
    ...
    public static void countingsort()throws Exception
    {
        ...
        for (int i = 0; i < count.length; count[i] = 0, i++, numcpr++);

        //Agora, o count[i] contem o numero de elemento iguais a i
        for (int i = 0; i < n; count[(int)p[i].getpeso()]++, i++, numcpr++);

        //Agora, o count[i] contem o numero de elemento menores ou iguais a i
        for(int i = 1; i < count.length; count[i] += count[i-1], i++, numcpr++);

        //Ordenando
        for(int i = n-1; i >= 0; ordenado[count[(int)p[i].getpeso()]-1] = p[i].clone(), count[(int)p[i].getpeso()]--, i--, numcpr++,nummov++);

        //Copiando para o array original
        for(int i = 0; i < n; p[i] = ordenado[i].clone(), i++, nummov++);

        imprimir(p,n);
    }

    public static int getMaior(Personagem[] p,int n) 
    {
        int maior =(int)p[0].getpeso();

        for (int i = 0; i < n; i++)
        {
            if(maior < (int)p[i].getpeso()){
                maior = (int)p[i].getpeso();
            }
        }
