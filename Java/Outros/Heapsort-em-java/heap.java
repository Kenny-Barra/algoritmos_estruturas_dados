...
public class heap
{
    ...
    public static void constroi(Personagem[] p,int tamHeap)throws Exception
    {
        for(int i = tamHeap; i > 1 && (p[i].getaltura() > p[i/2].getaltura() ||
        (p[i].getaltura() == p[i/2].getaltura() && p[i].getnome().compareTo(p[i/2].getnome())>0)); i /= 2)
        {
            numcpr+=2;
           swap(p,i, i/2);
        }
    ...
    public static void reconstroi(Personagem[] p,int n)throws Exception
    {
        int i=1;
        ...
        {
            numcpr++;
            int filho=getMaiorFilho(p,i,n);
            
            if(p[i].getaltura()<p[filho].getaltura()||
            (p[i].getaltura()==p[filho].getaltura() && p[i].getnome().compareTo(p[filho].getnome())<0))
            {
                numcpr++;
				swap(p,i, filho);
				i=filho;
    ...
    public static int getMaiorFilho(Personagem[] p,int i, int n)throws Exception{
        int filho;
        numcpr++;

        if(2*i==n || (p[2*i].getaltura() > p[2*i+1].getaltura() ||
        (p[2*i].getaltura() == p[2*i+1].getaltura() && p[2*i].getnome().compareTo(p[2*i+1].getnome())>0)))
        {
			filho=2*i;
		}//end if
        else 
