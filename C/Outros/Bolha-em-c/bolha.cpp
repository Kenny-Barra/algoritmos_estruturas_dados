...
void bubbleSort()
{
    for (int i = (nump - 1); i > 0; i--, numcpr++)
    {
        for (int j = 0; j < i; j++, numcpr++)
        {
            numcpr++;
            if (strcmp(personagens[j].anoNascimento,personagens[j + 1].anoNascimento)>0 ||
            (strcmp(personagens[j].anoNascimento,personagens[j + 1].anoNascimento)==0 && 
            strcmp(personagens[j].nome,personagens[j + 1].nome)>0))
            {
                nummov++;
                Personagem auxiliar = personagens[j + 1];
                personagens[j + 1] = personagens[j];
                personagens[j] = auxiliar;
            }
        }
    }
}
