...
void reconstroi(int i,int n);
int getMaiorFilho(int i, int n);
void heapSort();
...
void constroi(int tamHeap)
{
    for(int i = tamHeap; i > 1 && (personagens[i].altura > personagens[i/2].altura ||
    (personagens[i].altura == personagens[i/2].altura && strcmp(personagens[i].nome,personagens[i/2].nome)>0)); i /= 2)
    {
        numcpr+=2;
        swap(i,i/2);
    }
...
{       
    while(i<=(n/2))
    {
        ...
        (personagens[i].altura==personagens[filho].altura && strcmp(personagens[i].nome,personagens[filho].nome)<0))
        {
            numcpr++;
            swap(i, filho);
            i=filho;
...
int getMaiorFilho(int i, int n)
{
    ...
    if(2*i==n || (personagens[2*i].altura > personagens[2*i+1].altura ||
    (personagens[2*i].altura == personagens[2*i+1].altura && strcmp(personagens[2*i].nome,personagens[2*i+1].nome)>0)))
    {
        filho=2*i;
    }//end if
    else 
...
void heapSort()
{
    ...
    for(int tamHeap = 2; tamHeap <= nump; tamHeap++)
    {
        numcpr++;
        constroi(tamHeap);
    }
...
void salvarArq(double tempo)
{
    ...
    arq = fopen("matricula_insercaoparcial.txt", "wt");
